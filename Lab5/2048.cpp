  ...
  state select_best_move(const board &b) const {
    ...
    for (int i=0;i<4;i++) {
      ...
      if (move->assign(b)) {
        ...
        //  try sampling. 
        /*float expect_value_of_before_state = 0;
        int sample_count = 10;
        for( int i =0; i < sample_count;i++){
          ...
          Q = move->after_state();
          Q.popup();
          expect_value_of_before_state += estimate(Q);
          std::cout << estimate(Q) << " ";
        }
        move->set_value( move->reward() +  (expect_value_of_before_state / sample_count) );
        std::cout << (expect_value_of_before_state / sample_count) << "/" << estimate(move->after_state()) << std::endl;*/
